...
4. 使用 Chatflow 构建复杂 RAG（进阶）
   ...
   flowchart TD
       ...
       CLASSIFY -->|"SRE问题"| KB_HR["📚 知识检索<br/>SRE知识库"]
...
6. 常见问题与排查
   ...
   | 问题                   | 可能原因                       | 解决方案                              |
   | :--------------------- | :----------------------------- | :------------------------------------ |
   | 知识库索引一直"处理中" | Worker 服务异常                | `docker compose logs worker` 查看报错 |
   | 召回测试无结果         | Embedding 模型未配置           | 检查系统嵌入模型是否设置              |
   | 回答全是幻觉           | Prompt 约束不足 / 未关联知识库 | 强化 Prompt；确认 Context 已添加      |
   | 检索到但回答不完整     | 分块太小 / Top-K 太小          | 增大分块；增加 Top-K                  |
   | 上传文件报错           | 文件格式不支持 / 超大          | 检查格式；拆分为小文件                |
   | 应用响应超时           | 模型推理慢 / 网络问题          | 检查模型服务；增加超时配置            |
   | 升级后知识库报错       | Weaviate 版本不兼容            | 参考版本兼容表升级向量库              |
